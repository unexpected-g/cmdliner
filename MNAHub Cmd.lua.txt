--[[
	...
	Author: lxte
	Github: github.com/lxte/cmd
	Description: Base do script utilizada para MNAHub CMD
	
	Modifications by: MNAHub
	Current Version: 6.7
	================================
...
local Settings = ({
	...
	Theme = ({
	    ...
	    -- Frames:
	    Primary = Color3.fromRGB(7, 127, 253),     -- Fundo principal (AGORA É AZUL!)
	    Secondary = Color3.fromRGB(10, 80, 180),   -- Fundo secundário (azul mais escuro)
	    Actions = Color3.fromRGB(5, 60, 140),      -- Botões de ação
	    Component = Color3.fromRGB(8, 70, 160),    -- Componentes
	    Highlight = Color3.fromRGB(7, 127, 253),   -- Cor de destaque (mesmo azul)
	    ScrollBar = Color3.fromRGB(3, 40, 100),    -- Barra de rolagem

	    -- Text:
	    Title = Color3.fromRGB(255, 255, 255),     -- Título branco (contrasta com azul)
	    Description = Color3.fromRGB(200, 220, 255), -- Descrição azul clarinho

	    -- Outlines:
	    Shadow = Color3.fromRGB(0, 0, 0),          -- Sombra preta
	    Outline = Color3.fromRGB(5, 60, 140),      -- Borda azul escuro

	    -- Image:
	    Icon = Color3.fromRGB(255, 255, 255),      -- Ícones brancos
}),
...
local UI = (Services.Run:IsStudio() and script.Parent) or (function()
	if (game.PlaceId == 138747514708608 and not identifyexecutor) then
			...
			Window:AddToggle({
				...
				Callback = function(Toggle)
					...
					if Toggle then
						API:Notify({
							Title = "Success",
							Description = "Now everytime you run MNAHub Cmd other admin prompts wont interfere",
							Mode = "Success",
							Duration = 5,
						})
					end
...
Command.Add({
	Aliases = { "aimbot", "ab" },
	Description = "Aimbot features built into MNAHub Cmd",
	Arguments = {},
	Task = function()
...
Command.Add({
	...
	Task = function()
		...
		if (Tab) then
			...
			Tutorial.Invite.Text = "https://discord.gg/r3bBmHqGy2"
		end
	end,
})
...
Command.Add({
	...
	Description = "Gets rid of MNAHub Cmd",
	Arguments = {},
	Task = function()
...
Command.Add({
	...
	Description = "Allows your target to use MNAHub Cmd's commands (using chat)",
	Arguments = {
		{ Name = "Target", Type = "Player" },
	},
	Task = function(Input)
		for Index, Admin in next, GetPlayer(Input) do
			...
			Chat(Format('/w %s You are now whitelisted to MNAHub Cmd, prefix is "%s"', Admin.Name, Settings.ChatPrefix))
		end
	end,
})
...
if (Check("File")) then
	...
	for Index, Folder in next, ({ "MNAHubCmd", "MNAHubCmd/Logs", "MNAHubCmd/Plugins" }) do
		if (not isfolder(Folder)) then
			makefolder(Folder);

			if (Folder == "MNAHubCmd") then
	...
	for Index, File in next, listfiles("MNAHubCmd/Plugins") do
...
do
	...
	if (Check("File") and not isfile("MNAHubCmd/Settings.json")) then
		SaveSettings();
	end
	...
	if (Settings.Toggles.Developer) then
		...
		if (Interface and API) then
			...
			API:Notify({
				...
				Description = "If you would like to reload MNAHub Cmd rejoin or enable Developer mode!",
				Duration = 5,
				Type = "Error",
			})
...
API:Notify({
	Title = "Welcome to MNAHub CMD (NEW UPDATE)",
	Description = Format(
